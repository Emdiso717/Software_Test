<template>
    <el-scrollbar height="100%" style="width: 100%;">
        ...
        <div style="width:30%;margin:0 auto; padding-top:5vh;">
            ...
            <el-button style="margin-left: 200px;margin-top: -60px;" type="primary" @click="QueryBorrows">查询</el-button>

        </div>

        <!-- 结果表格 -->
        <el-table v-if="isShow" :data="fitlerTableData" height="600" :formatter="dateFormat"
            ...
            <el-table-column prop="returnTime" label="归还时间" sortable >
                <template #default="scope">
                    {{scope.row.returnTime !== 0 ? scope.row.returnTime : "未归还" }}
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        async QueryBorrows() {
            ...
            borrow.borrowTime = dayjs(borrow.borrowTime).format('YYYY.MM.DD HH:mm')
            if (borrow.returnTime != 0) {
                borrow.returnTime=dayjs(borrow.returnTime).format('YYYY.MM.DD HH:mm')   
            }
